...
class QuoteManager {
    ...
    initializeEventListeners() {
        ...
        categorySelect.addEventListener('change', (event) => {
            ...
            this.filterQuotes(selectedCategory); // Call the filter function here
        });
    ...
    filterQuotes(selectedCategory) {
        ...
        const filteredQuotes = this.quotes.filter(quote => quote.category === selectedCategory || selectedCategory === 'all');

        // Check if there are any filtered quotes
        if (filteredQuotes.length === 0) {
            quoteDisplay.innerHTML = '<p>No quotes available in this category.</p>';
            return; // Exit the function
        }

        // Map over the filtered quotes and display them
